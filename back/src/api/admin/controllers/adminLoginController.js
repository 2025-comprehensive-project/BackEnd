const db = require('../../../config/dbConnect'); // 데이터베이스 연결 설정
const bcrypt = require('bcrypt'); // 비밀번호 해싱을 위한 bcrypt 라이브러리
const jwt = require('jsonwebtoken'); // JWT 토큰 생성을 위한 라이브러리
const nodemailer = require('nodemailer'); // 이메일 전송을 위한 Nodemailer
const client = require('../../../config/redisClient'); // Redis 클라이언트
...
const sendPasswordResetLink = async (req, res, next) => {
    ...
    try {
        ...
        await client.setEx(cooldownKey, 600, '1');
        await client.setEx(tokenKey, 900, token);
...
// 비밀번호 재설정
// 현재 프론트 페이지가 없어 GET 요청을 사용함
// 실제로는 POST 요청으로 변경 필요
const resetPassword = async (req, res, next) => {
    ...
    try {
        ...
        await client.del(`flapper:reset:cooldown:${email}`);
