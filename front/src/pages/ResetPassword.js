...
function ResetPassword() {
  ...
  const email = queryParams.get('email'); // URL 쿼리 파라미터에서 이메일 가져오기

  useEffect(() => {
    // 토큰이나 이메일이 없다면 홈으로 리다이렉트
    if (!token || !email) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // 실제 API 엔드포인트와 파라미터는 백엔드 구현에 맞게 수정 필요
