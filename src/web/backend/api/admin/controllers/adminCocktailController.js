...
const addCocktail = async (req, res, next) => {
  ...
  try {
    ...
    if (!ingredient1_id || !ingredient2_id) {
      logger.error('❌ 필수 재료 체크 실패: ingredient1_id 또는 ingredient2_id 없음');
      return next(createError(400, '❌ 1번, 2번 재료는 필수입니다.', 'MISSING_BASE_INGREDIENT'));
    }

    // ✅ [2] 기주 ID(1~8) 체크
    const baseSpiritIds = [1,2,3,4,5,6,7,8];
    if (!baseSpiritIds.includes(Number(ingredient1_id))) {
      logger.error(`❌ 기주 ID 체크 실패: 주어진 값들 → ${ingredient1_id}`);
      return next(createError(400, '❌ 1번 재료는 반드시 기주(1~8번)여야 합니다.', 'INVALID_BASE_INGREDIENT'));
    }
